    ...
    : QWidget(pParent)
{
    ...
    connect(m->pPdfJsBridge, &PdfJsBridge::selectionChanged, this, &QPdfWidget::onSelectText);
    m->ready = false;
...
void QPdfWidget::keyPressEvent(QKeyEvent *event)
{
    qDebug() << event->key();
...
void QPdfWidget::onSelectText()
{
    save_phaze = m->pPdfJsBridge->selectedText();
}
...
QStringList QPdfWidget::getSave_phazes() const
{
    return save_phazes;
}
